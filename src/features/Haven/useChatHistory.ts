...
const STORAGE_KEY = 'haven-chat-history-v5'
...
export function useChatHistory() {
  ...
  const upsertSession = useCallback((id: string, memberKey: string, memberName: string, messages: Message[]) => {
    if (!messages.some(m => m.role === 'user')) return
    const firstUserMsg = messages.find(m => m.role === 'user')?.content ?? ''
    ...
    const session: ChatSession = {
      ...
      summary: firstUserMsg.slice(0, 120),
      favorited: idx >= 0 ? existing[idx].favorited : false,
      messages,
    }
    if (idx >= 0) {
      ...
      persistHistory([...existing, session])
    }
  }, [])
  ...
  return { getSessionsForMember, saveSession, upsertSession, deleteSession, toggleFavorite, clearAllForMember }
}
